...
<section class="single-post-content">
    <div class="container">
        <div class="row">
            <?php
            $id = $_GET['id'];
            $query = mysqli_query($db, "SELECT blog.title,blog.description,blog.content,blog.video,blog.created_at,blog.image,categories.title as name FROM  blog LEFT JOIN categories ON blog.cat_id = categories.id WHERE blog.id=$id");
            ...
            <div class="col-md-9 post-content" data-aos="fade-up">
                ...
                <div class="single-post">
                    <div class="post-meta"><span class="date"><?=$row['title']?></span> <span class="mx-1">&bullet;</span> <span><?=date("Y-m-d",strtotime($row['created_at'])) ?></span></div>
                    <h1 class="mb-5"><?=$row['name']?></h1>
                    ...
                    <iframe width="400" height="400" src="<?= $row['video'] ?>" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" ></iframe>
